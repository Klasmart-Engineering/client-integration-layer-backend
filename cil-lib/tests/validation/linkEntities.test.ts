...
import { OnboardingRequest, Action, LinkEntities, Organization, EntitiesToLink, User, School, Class, Entity } from "../../src/lib/protos";
  ...
  export const INVALID_LINK_ENTITY: LinkEntitiesTestCase[] = [
    {
      ...
      errorExpectation: new ErrorExpectation("If Action is of type 'LINK', payload must be 'LinkEntities'")
    },
    {
        scenario: "LinkEntities missing 'ORGANIZATION', 'SCHOOL', 'CLASS' or 'USER'",
        linkedEntities: new LinkEntities(),
        errorExpectation: new ErrorExpectation("A request to LinkEntities must include either an 'ORGANIZATION', 'SCHOOL', 'CLASS' or 'USER'")
    },
    {
      scenario: "Organization ID provided and the entity identifier provided did not match",
      linkedEntities: (() => {
      ...
      errorExpectation: new ErrorExpectation("Found that the Organization ID provided and the entity identifier provided did not match despite the entity being set to 'ORGANIZATION'")
  },
  {
    ...
    errorExpectation: new ErrorExpectation("Expected to find data however it was undefined", "validation")
  },
  {
...
export const VALID_LINK_ENTITIES: LinkEntitiesTestCase[] = [
  {
    ...
    linkedEntities: (() => {
        ...
        entities.setEntities(entitiesToLink(Entity.ORGANIZATION, [uuidv4(), uuidv4()]));
        return entities;
    })()
  },
  {
  ...
  describe('linkEntities validation', () => {
    ...
    beforeEach(() => {
      ...
      INVALID_LINK_ENTITY.forEach(({ scenario, linkedEntities, errorExpectation }) => {
        it(`should fail when ${scenario}`, async () => {
            
          try {
            const resp = await ValidationWrapper.parseRequest(onboardingRequest(linkedEntities), LOG_STUB);
        ...
        it(`should succeed when ${scenario}`, async () => {
          ...
          switch(linkedEntities?.getTargetCase()) {
            ...
            case(LinkEntities.TargetCase.SCHOOL): {
              const schoolExternalId = linkedEntities?.getSchool()?.getExternalUuid();
            ...
            case(LinkEntities.TargetCase.ORGANIZATION): {
              const orgExternalId = linkedEntities?.getOrganization()?.getExternalUuid();
  ...
  function identifiers(entities: EntitiesToLink | undefined, externalId?: string): string[] {
